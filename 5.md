...
tool.php の先頭に以下のコードを挿入します。

<?php
// 認証処理（Authorizationヘッダーの静的トークン確認）
...
if (!preg_match('/Bearer\s+(.*)$/i', $authHeader, $matches) || $matches[1] !== $expectedToken) {
...
require_once __DIR__ . '/../tool_common.php';
...
namespace cms\site\hobby\package;

use cms\cmslib\CmsStringUtil;
use cms\site\hobby\common\HobbyBaseApi;

class Tool extends HobbyBaseApi
{
    /**
     * コンストラクタ
     */
    public function __construct($logger = null)
    {
        ...
        // 認証チェック（ここで全処理に対して強制する）
        $authHeader = $_SERVER['HTTP_AUTHORIZATION'] ?? '';
        $expectedToken = 'Bearer abcdefg123456'; // ← ここは実際のTokenに置き換える
    ...
    /**
     * 翻訳
     * wovn翻訳を実施し、結果取得用の情報を通知する
     */
    private function translate($prams = [])
    {
        $this->logger->info(get_called_class().'::'.__FUNCTION__, $prams);

        if (!isset($prams['site']) || !isset($prams['ip'])) {
            return $this->returnNG('param data not ip or site');
        }

        if (!$this->isPost()) {
            return $this->returnNG('Illegal API call');
        }
...
$authorized = false;
$static_token = 'your_static_token_here'; // ← トークンを任意で変更可
