...
もちろん、以下は VPCを使用しない構成におけるネットワーク設計の要点（日本語版） の要約です：


---

🌐 VPCなし構成のネットワーク設計ポイント

設計観点	内容

アクセス制御	S3バケットポリシー、API GatewayのリソースポリシーでIP制限やサービス指定を行う
外部公開の保護	CloudFrontやAPI Gatewayに AWS WAF を連携して、不正アクセス・DDoS対策を実施
通信経路の最適化	CloudFront → API Gateway → Lambda（→ S3やDynamoDB）などの経路を明確に設計
HTTPS対応	Route 53 + CloudFront + ACMでカスタムドメイン＋SSL証明書を構成
ログ・モニタリング	CloudWatch LogsやS3へのアクセスログを有効化してネットワークアクセスを可視化
...
セキュリティグループやNACLが使えないため、リソースポリシー/IAMベースの制御が中心

RDSやElastiCacheなどのVPC内リソースとは通信不可

Lambda関数は現在デフォルトでVPCに配置されるため、外部アクセスには パブリックサブネット設定 が必要
